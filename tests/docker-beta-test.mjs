...
for (const pm of PACKAGE_MANAGERS) {
    ...
    test(`${pm}: bundle resolvable`, () => {
        try {
            req.resolve('ringfence/dist/ringfence.mjs', { paths: [project] });
        } catch {
            throw new Error('bundle not resolvable from ' + project);
        }
    });
    ...
    test(`${pm}: install triggers ringfence sandbox`, () => {
        ...
        if (out.includes('ringfence') || out.includes('masking') || out.includes('sandbox')) {
            ...
            throw new Error(
                `No ringfence output and package not installed.\nExit: ${r.status}\n${(r.stdout || '').slice(-400)}\n${(r.stderr || '').slice(-400)}`,
            );
        }
    });
    ...
    test(`${pm}: zod installed in node_modules`, () => {
        ...
        if (!existsSync(`${project}/node_modules/zod`))
            throw new Error('zod not found after install');
    });
}
